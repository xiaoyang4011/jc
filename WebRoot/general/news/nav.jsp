...
<script type="text/javascript" charset="utf-8">
	...
	$(function() {
		...
		treegrid = $('#treegrid').treegrid({
			url : '${pageContext.request.contextPath}/nav!treegrid.do',
			toolbar : [ {
				text : '展开',
				iconCls : 'icon-redo',
				handler : function() {
	...
	function edit() {
		...
		if (node) {
			var p = parent.dj.dialog({
				title : '编辑导航',
				href : '${pageContext.request.contextPath}/nav!navEdit.do',
				width : 460,
				height : 220,
				buttons : [ {
					text : '编辑',
					handler : function() {
						var f = p.find('form');
						f.form('submit', {
							url : '${pageContext.request.contextPath}/nav!edit.do',
							success : function(d) {
				...
				onLoad : function() {
					...
					var ptree = pid.combotree({
						lines : true,
						url : '${pageContext.request.contextPath}/nav!doNotNeedSession_treeRecursive.do',
	...
	function append() {
		var p = parent.dj.dialog({
			title : '添加导航',
			href : '${pageContext.request.contextPath}/nav!navAdd.do',
			width : 460,
			height : 220,
			buttons : [ {
				text : '添加',
				handler : function() {
					var f = p.find('form');
					f.form('submit', {
						url : '${pageContext.request.contextPath}/nav!add.do',
						success : function(d) {
							var json = $.parseJSON(d);
							if (json.success) {
								treegrid.treegrid('reload');
			...
			onLoad : function() {
				...
				var ptree = pid.combotree({
					lines : true,
					url : '${pageContext.request.contextPath}/nav!doNotNeedSession_tree.do'
				});
			}
		});
	}
	function remove() {
		...
		if (node) {
			parent.dj.messagerConfirm('询问', '您确定要删除【' + node.cname + '】？', function(b) {
				if (b) {
					$.ajax({
						url : '${pageContext.request.contextPath}/nav!delete.do',
						data : {
							cid : node.cid
						},
						cache : false,
